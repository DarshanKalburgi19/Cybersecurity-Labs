...
## Administrator Password Disclosure via IDOR on Account Page
...
Modify the id parameter to reference the administrator account:
...
Send the request and inspect the response source.
...
The response contains the administrator’s password value in the HTML source (masked in the UI but visible in the response body).

Using the extracted password, the attacker can authenticate as the administrator and delete user carlos.
