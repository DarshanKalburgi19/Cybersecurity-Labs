# Lab 6: Privilege Escalation via Method-Based Access Control Bypass
...
## Steps to Reproduce
1. Authenticate as an administrator and observe the request used to upgrade a user’s role.
2. Log out and authenticate as a low-privileged user.
3. Send the following request manually:
   `GET /admin-roles?username=wiener&action=upgrade HTTP/2`
4. Observe that the request succeeds and the user `wiener` is upgraded to administrator, despite lacking the required privileges.

## Proof of Concept
The above request successfully escalates the attacker-controlled account from a normal user to administrator without authorization by switching the expected `POST` method to a `GET` method.
